...
import { TopPerformer } from '../data/mockInstitutionsData'

interface InstitutionTopPerformersChartProps {
  data: TopPerformer[]
}

const COLORS = ['#09B0B6', '#05647A', '#266BAC', '#3DCCD0', '#63A7D8']

export function InstitutionTopPerformersChart({ data }: InstitutionTopPerformersChartProps) {
  return (
    <Card className="border-t-4 border-t-[#09B0B6] ">
      <CardHeader>
        ...
        <CardDescription className="text-xs md:text-sm">
          Top institutions by trips completed
        </CardDescription>
      </CardHeader>
      <CardContent className="px-2" style={{ padding: '0px' }}>
        <div className="space-y-4">
          <ResponsiveContainer  width="100%" height={450} minHeight={250}>
            <BarChart data={data}>
